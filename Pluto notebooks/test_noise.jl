...
# ╔═╡ 2cb7aeee-d232-44b3-9d7a-085991ba1a82
...
map(0:2^nqubits -1) do i
	...
	push!(states_bin1, out)
	md"bit-string for $i is $out"
end
end

# ╔═╡ 3bd8acf6-4bf8-4deb-a8fd-84ef2f5b7821


# ╔═╡ 598fa695-21ca-40f9-afd2-65d2f2fcc5f5


# ╔═╡ 6dee586e-f7bd-45fc-9e69-f31ef0035c15
...
begin
measures_new2 = run(qubits2,circ_depth2,n_shots);
states_bin22 = []
map(0:2^qubits2 -1) do i
	out = reverse(digits(i; base=2, pad=qubits2))
	push!(states_bin22, out)
end
times_state_appear22 = counter(eachrow(measures_new2));
end

# ╔═╡ 14b8736e-7d5f-42fb-89a7-1e744d136169
begin
scatter(1:2^qubits2,repeat([1],2^qubits2),ylim=(0.85,1.15), color = :bluesreds, 
		ms = [100*times_state_appear22[i]/n_shots for i in states_bin22],
		title="Speckle plot for $qubits2 qubits and $circ_depth2 layers circuit",
		legend = :false)
xticks!([1:2^qubits2;], [prod(string.(i)) for i in states_bin22],xrotation=90)
end
#ms = [100*times_state_appear2[i]/n_shots for i in states_bin2]
#ylim=(0.85,1.15)

# ╔═╡ c8db7d32-c0e5-415e-8b07-36dcaf29c535
begin
scatter(1:2^qubits2,repeat([1],2^qubits2),ylim=(0.85,1.15), color = :bluesreds, 
		c=colormap("Reds",15,mid =0.7),
	    ms = [100*times_state_appear22[i]/n_shots for i in states_bin22],
		title="Speckle plot for $qubits2 qubits and $circ_depth2 layers circuit",
		legend = :false)
xticks!([1:2^qubits2;], [prod(string.(i)) for i in states_bin22],xrotation=90)
...
#c=colormap("Blues",15,mid=0.2)

# ╔═╡ 9fae96d2-24b1-43ee-b6c4-618eb1e3f8a9
#begin
#scatter(1:2^qubits2,repeat([1],2^qubits2),ylim=(0.85,1.15), color = :bluesreds, 
#	    ms = [100*times_state_appear_noise[i]/n_shots for i in states_bin_noise],
#		title="Speckle plot for $qubits2 qubits and $circ_depth2 layers circuit with bit-flip",
#		legend = :false)
#xticks!([1:2^qubits2;], [prod(string.(i)) for i in states_bin_noise],xrotation=90)
#end
...
function run_noise(N, depth,nshots=1024)
	#random circuit.
	gates = Vector{Tuple}[]
...
# ╔═╡ b2692c76-ebc3-41c1-88dd-daae68001cc3
rand()

# ╔═╡ fcf65016-bb11-471c-b96a-ee526c2f4b4c
measures_noise = run_noise(nqubits,circ_depth);

# ╔═╡ 5be78a00-5bf9-40d9-b266-35c696abc7c4
#begin
#states_bin = []
#map(0:2^nqubits -1) do i
#	out = reverse(digits(i; base=2, pad=nqubits))
...
# ╔═╡ db3e0cca-88e8-402f-bc54-86e089d44c10
#times_state_appear_noise = counter(eachrow(measures_noise));

# ╔═╡ 7284e949-7681-4b39-93a3-6f60001438ee
#map(1:2^nqubits) do i
#	state = states_bin[i]
#	ntimes_noise = times_state_appear_noise[state]
	#md"The state $state shows $ntimes in $nshots shots"
#end

# ╔═╡ 412b73ff-ca89-4439-8f3d-12b519386847
#begin
...
# ╔═╡ 9ed34145-d956-47f4-a4f3-9fccc2285796
begin
measures_new_noise = run_noise(qubits2,circ_depth2,n_shots);
states_bin_noise = []
map(0:2^qubits2 -1) do i
	out = reverse(digits(i; base=2, pad=qubits2))
	push!(states_bin_noise, out)
end
times_state_appear_noise = counter(eachrow(measures_new_noise));
end

# ╔═╡ ff51262d-d026-4221-80ac-68754a200da3
begin
scatter(1:2^qubits2,repeat([1],2^qubits2),ylim=(0.85,1.15), color = :bluesreds, 
		c=colormap("Reds",10,mid =0.7),
	    ms = [100*times_state_appear_noise[i]/n_shots for i in states_bin_noise],
		title="Speckle plot for $qubits2 qubits and $circ_depth2 layers circuit with bit-flip",
		legend = :false)
xticks!([1:2^qubits2;], [prod(string.(i)) for i in states_bin_noise],xrotation=90)
...
# ╔═╡ 5d5bc880-958a-44ce-a1c4-cf23e5a11dc0
md"""
# Task 4
"""

# ╔═╡ ee634362-75e0-494f-8047-f558cd6836fe
...
# ╔═╡ 05ac95eb-4720-4388-aa18-b4ca59efc368
begin
measures_gate_error = run_gate_error(qubits2,circ_depth2,0.5,n_shots);
states_bin_gate = []
map(0:2^qubits2 -1) do i
	out = reverse(digits(i; base=2, pad=qubits2))
	push!(states_bin_gate, out)
end
times_state_appear_gate_error = counter(eachrow(measures_gate_error));
end

# ╔═╡ da181325-2c2e-4223-8980-c6b2e1847bdb


# ╔═╡ da019933-df6f-43a3-9612-b603d0c58c65
times_state_appear_gate_error 

# ╔═╡ be3a2cbb-13cd-429f-a615-64937fb21507
2^qubits2

# ╔═╡ 4f527a7e-3d4c-432f-bb76-af8def2d562d
states_bin_gate

# ╔═╡ 7d4bc674-7ca5-4861-9860-efe3e0643e5f
ms = [times_state_appear_gate_error[i]/n_shots for i in states_bin_gate]

# ╔═╡ 785cef1c-35d3-4fc5-a27e-53ac5cc1a518
px_sum = [ (2^qubits2/n_shots)*(ms[i]) for i in 1:2^qubits2 ]

# ╔═╡ b2743c4d-6f22-4a9c-8dce-e0c292c9370a
sum(px_sum)

# ╔═╡ 01317318-59b4-4169-b870-d5ed8ee1eb83
ms2 = [times_state_appear_gate_error[i] for i in states_bin_gate]

# ╔═╡ 15a6a8cf-0dd6-4675-b346-cc58618247ff


# ╔═╡ 6bcb0dfd-d116-4578-a278-1d6f4dab925f
qubits2

# ╔═╡ 5f9254d9-b074-4a6e-946b-10b01375deae
2^qubits2

# ╔═╡ 9c26b7c7-febf-4871-bfde-8a76e99af2b2
m3 =[1/2^qubits2, 1/2^qubits2, 1/2^qubits2, 1/2^qubits2,1/2^qubits2, 1/2^qubits2, 1/2^qubits2, 1/2^qubits2]

# ╔═╡ f890253a-d438-44d7-a946-bb2ad13a4092
#suma = [ (2^qubits2/n_shots)*(m3[i]) for i in 1:2^qubits2 ]

# ╔═╡ 0ae50ba1-9c01-434b-b012-cb9fd20e2a3c
#sum(suma)

# ╔═╡ 4ca117be-3ea5-4b29-a5fd-21d502d8d65d
#ms3 = [times_state_appear2[i]/n_shots for i in states_bin22]

# ╔═╡ d4d58133-6de8-49dd-96a6-9e5c019cc037
n_shots
